...
async fn create_list_rotate_validate_tab() {
    ...
    let body_bytes = to_bytes(resp.into_body()).await.expect("Failed to read response body");
    let created: TabResponse = serde_json::from_slice(&body_bytes).expect("Failed to parse JSON response");
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let body_bytes = to_bytes(resp.into_body()).await.expect("Failed to read response body");
    let listed: Vec<TabResponse> = serde_json::from_slice(&body_bytes).expect("Failed to parse JSON response");
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let body_bytes = to_bytes(resp.into_body()).await.expect("Failed to read response body");
    let rotated: VirtualIPResponse = serde_json::from_slice(&body_bytes).expect("Failed to parse JSON response");
    ...
    assert_eq!(resp.status(), StatusCode::OK);
    let body_bytes = to_bytes(resp.into_body()).await.expect("Failed to read response body");
    let validation: ValidationResponse = serde_json::from_slice(&body_bytes).expect("Failed to parse JSON response");
